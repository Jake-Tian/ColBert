...
def tensorize_triples(query_tokenizer, doc_tokenizer, queries, positives, negatives, bsize):
    ...
    # Triple: (q, d+, d-)
    # where q is a query, d+ is a positive document, and d- is a negative document.
    N = len(queries)
    Q_ids, Q_mask = query_tokenizer.tensorize(queries)
    D_ids, D_mask = doc_tokenizer.tensorize(positives + negatives)
    # Reshape D_ids and D_mask to (2, N, doc_maxlen)
    # D_ids[0] and D_mask[0] are positive documents, D_ids[1] and D_mask[1] are negative documents
...
def _sort_by_length(ids, mask, bsize):
    """Sort the tensor by length and return sorted ids, mask, and reverse indices."""
    if ids.size(0) <= bsize:
...
def _split_into_batches(ids, mask, bsize):
    """Split the tensor into batches of size bsize."""
    batches = []
